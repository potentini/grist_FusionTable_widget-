...
- Détecte automatiquement les tables source dont les noms correspondent à `Niveau1`, `Niveau2` et `Niveau3`.
...
3. Connectez cette première instance du widget à n'importe quelle table du document. Le widget utilise l'API document pour détecter les tables nommées `Niveau1`, `Niveau2` et `Niveau3`, puis les lire.
...
Les tables source sont détectées par leur nom (`Niveau1`, `Niveau2`, `Niveau3`, avec une tolérance sur la casse et les séparateurs). Les champs métier par défaut correspondent aux classes Python Grist `Niveau1`, `Niveau2` et `Niveau3` du document : `Projet`, `DateDebut_Prj`, `DateFin_Prj`, `Statut_Prj`, `Responsable_Prj`, `Progression_Prj`, `Tache`, `DateDebut_Tache`, `DateFin_Tache`, `Statut_Tache`, `Avancement_Tache`, `Sous_Tache`, `DateDebut_ST`, `DateFin_ST`, `Statut_ST` et `Responsable_ST`. Adaptez les options du widget si votre document utilise d'autres noms.
...
Dans les options JSON du widget Grist, vous pouvez remplacer `sources` si les noms de tables ou de colonnes changent. Dès que `sources` est fourni, la détection automatique par `Niveau1`, `Niveau2` et `Niveau3` est désactivée. Exemple minimal pour le niveau 1 :
